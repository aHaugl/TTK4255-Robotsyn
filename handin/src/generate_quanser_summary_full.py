...
# from quanser_fast import


def generate_quanser_summary(trajectory, residuals, detections, p_args):
    #
    ...
    plt.tight_layout()
    plt.savefig('../output/out_histogram.png')
    ...
    p = trajectory[:, p_args]
    vis_yaw = p[:, 0] + enc_yaw[0] - p[0, 0]
    vis_pitch = p[:, 1] + enc_pitch[0] - p[0, 1]
    vis_roll = p[:, 2] + enc_roll[0] - p[0, 2]

    vis_fps = 16
    enc_frame = enc_time*vis_fps
    vis_frame = np.arange(p.shape[0])
    ...
    plt.tight_layout()
    plt.savefig('../output/out_trajectory.png')
